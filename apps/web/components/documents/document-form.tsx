...
export function DocumentForm({ type }: DocumentFormProps) {
  ...
  const contactPlaceholder = config.contactFilter === 'vendor' ? 'Search vendors...' : 'Search customers...';
  ...
  async function handleSubmit() {
    ...
    startTransition(async () => {
      ...
      if (result.success && result.data) {
        ...
        setError(result.error || `Failed to create ${config.label.toLowerCase()}`);
      }
    });
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">New {config.label}</h1>
          <p className="text-muted-foreground">Create a new {config.label.toLowerCase()} draft.</p>
        </div>
      </div>
          ...
          <div className="rounded-xl border bg-card p-6 space-y-4">
            <div>
              ...
              <textarea
                ...
                placeholder={`Notes visible on the ${config.label.toLowerCase()}...`}
                rows={3}
                className="mt-1 w-full rounded-lg border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary"
              />
            </div>
            <div>
              ...
              <textarea
                ...
                placeholder={`Internal notes (not visible on ${config.label.toLowerCase()})...`}
                rows={2}
                className="mt-1 w-full rounded-lg border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary"
              />
            </div>
        ...
        <div className="space-y-6">
          <div className="sticky top-6 rounded-xl border bg-card p-6">
            ...
            <p className="mt-2 text-center text-xs text-muted-foreground">
              {config.label} will be created as a draft.
            </p>
          </div>
        </div>
      </div>
