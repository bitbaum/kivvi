...
export default async function EditPurchaseInvoicePage({ params }: PageProps) {
  ...
  if (!doc || doc.type !== 'purchase_invoice') {
    notFound();
  }

  if (doc.status !== 'draft') {
    redirect(`/purchasing/purchase-invoices/${id}`);
  }
    ...
    <div className="space-y-6">
      <Breadcrumb
        items={[
          ...
          { label: 'Purchase Invoices', href: '/purchasing/purchase-invoices' },
          { label: doc.number, href: `/purchasing/purchase-invoices/${id}` },
          { label: 'Edit' },
        ]}
      />
      <EditDocumentForm
