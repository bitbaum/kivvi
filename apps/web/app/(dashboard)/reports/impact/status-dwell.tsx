...
export async function StatusDwellSection({
  data,
}: {
  data: StatusDwellEntry[];
}) {
  ...
  return (
    <div className="rounded-xl border bg-card p-6">
      ...
      <div className="space-y-3">
        ...
        {active.map((d) => {
          const barPct =
            maxAvg > 0 ? Math.round((d.avgDays / maxAvg) * 100) : 0;
          ...
          return (
            ...
            >
              <span className="font-medium">
                {ti(
                  (STATUS_I18N_KEY[d.status] ?? d.status) as Parameters<
                    typeof ti
                  >[0],
                )}
              </span>
