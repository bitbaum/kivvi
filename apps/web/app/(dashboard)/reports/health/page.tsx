  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-4">
        <Link
          href="/reports"
          className="flex h-9 w-9 items-center justify-center rounded-lg border bg-card hover:bg-accent"
        >
  ...
  const metricCards = [
    {
      ...
      trend: metrics.profitMargin >= 20 ? 'excellent' : metrics.profitMargin >= 10 ? 'good' : 'poor',
    },
    {
      ...
      trend: metrics.conversionRate >= 30 ? 'excellent' : metrics.conversionRate >= 15 ? 'good' : 'poor',
    },
    {
      label: t('avgInvoice'),
    ...
    },
    {
      ...
      trend: metrics.avgDaysToPayment <= 15 ? 'excellent' : metrics.avgDaysToPayment <= 30 ? 'good' : 'poor',
    },
    {
      ...
      trend: metrics.cashFlowRatio >= 120 ? 'excellent' : metrics.cashFlowRatio >= 100 ? 'good' : 'poor',
    },
    {
      ...
      trend: metrics.customerRetentionRate >= 80 ? 'excellent' : metrics.customerRetentionRate >= 60 ? 'good' : 'poor',
    },
  ];
...
async function ChartsSection() {
  ...
  const [salesReport, agingReport] = await Promise.all([
    getSalesReport(db, companyId, sixMonthsAgo.toISOString().split('T')[0], new Date().toISOString().split('T')[0]),
