...
import { Loader2, CheckCircle2, AlertTriangle, Hash, FileText, BookOpen, CalendarX } from 'lucide-react';
...
export function DataRepairPanel({ initialStatus }: { initialStatus: RepairStatus | null }) {
  ...
  async function handleRepairSequences() {
    ...
    if (result.success && result.data) {
      ...
      setStatus((prev) => prev ? { ...prev, sequences: updatedSeqs.length > 0 ? updatedSeqs : prev.sequences } : prev);
    } else {
    ...
    if (result.success && result.data) {
      ...
      setStatus((prev) =>
        prev
          ? {
              ...prev,
              sentInvoicesBefore2026: prev.sentInvoicesBefore2026 - result.data!.updatedInvoices,
              sentPurchaseInvoicesBefore2026: prev.sentPurchaseInvoicesBefore2026 - result.data!.updatedPurchaseInvoices,
            }
          : prev
      );
    } else {
    ...
    if (result.success && result.data) {
      ...
      setStatus((prev) =>
        prev
          ? {
              ...prev,
              totalJournalEntries: prev.totalJournalEntries + result.data!.invoiceEntries + result.data!.purchaseEntries,
              documentsWithoutJournalEntries: Math.max(0, prev.documentsWithoutJournalEntries - result.data!.invoiceEntries - result.data!.purchaseEntries),
            }
          : prev
      );
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Invoice Statuses */}
      {(status.sentInvoicesBefore2026 > 0 || status.sentPurchaseInvoicesBefore2026 > 0) && (
        <div className="rounded-xl border bg-card p-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="rounded-lg border bg-background p-2">
          ...
          <div className="mb-4 space-y-2 text-sm">
            ...
            <p>{t('sentPurchaseInvoices', { count: status.sentPurchaseInvoicesBefore2026 })}</p>
          </div>
        ...
        <div className="mb-4 space-y-2 text-sm">
          ...
          {status.documentsWithoutJournalEntries > 0 && (
            <p className="text-amber-600">
              {t('missingEntries', { count: status.documentsWithoutJournalEntries })}
            </p>
          )}
        </div>
      ...
      {status.paidInvoicesWithoutPaidDate > 0 && (
        <div className="rounded-xl border bg-card p-6">
          ...
          <div className="mb-4 space-y-2 text-sm">
            <p className="text-amber-600">
              {t('paidDatesCount', { count: status.paidInvoicesWithoutPaidDate })}
            </p>
          </div>
          ...
          >
            ...
            {repairingPaidDates ? t('repairingPaidDates') : t('repairPaidDates')}
          </button>
        </div>
      )}
      ...
      {!hasIssues && (
        <div className="rounded-xl border border-green-200 bg-green-50 p-6 dark:border-green-900/50 dark:bg-green-900/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-green-800 dark:text-green-400">{t('noIssuesFound')}</h3>
              <p className="text-sm text-green-700 dark:text-green-500">{t('allGood')}</p>
            </div>
          </div>
        </div>
      )}
