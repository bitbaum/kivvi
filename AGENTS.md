...
- **Deploy**: push to the default branch (`main`) → CI-gated background deploy to a self-hosted Hetzner PostgreSQL box (behind Caddy) via the Loki push-deploy hook. Prod only receives commits CI verified green.
