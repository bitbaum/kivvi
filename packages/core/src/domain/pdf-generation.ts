...
export async function generateInvoicePdf(data: InvoicePdfData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc
      ...
      .text(data.companyName, MARGIN_LEFT, MARGIN_TOP);
    ...
    const cols = {
      ...
      desc: { x: MARGIN_LEFT + 30, w: CONTENT_WIDTH - 30 - 55 - 65 - 45 - 45 - 70 },
      qty: { x: 0, w: 55 },
      price: { x: 0, w: 65 },
      vat: { x: 0, w: 45 },
      disc: { x: 0, w: 45 },
    ...
    doc.text('Menge', cols.qty.x, tableTop, { width: cols.qty.w, align: 'right' });
    doc.text('Einzelpreis', cols.price.x, tableTop, { width: cols.price.w, align: 'right' });
    doc.text('MWST', cols.vat.x, tableTop, { width: cols.vat.w, align: 'right' });
    doc.text('Rabatt', cols.disc.x, tableTop, { width: cols.disc.w, align: 'right' });
    ...
    for (const item of data.items) {
      ...
      doc.text(formatSwissAmount(item.unitPrice, ''), cols.price.x, rowY, { width: cols.price.w, align: 'right' });
      doc.text(`${item.vatRate}%`, cols.vat.x, rowY, { width: cols.vat.w, align: 'right' });
      doc.text(discountStr, cols.disc.x, rowY, { width: cols.disc.w, align: 'right' });
      doc.font(FONT_BOLD).text(formatSwissAmount(item.total, ''), cols.total.x, rowY, { width: cols.total.w, align: 'right' });
    ...
    doc.font(FONT_REGULAR).fontSize(9).fillColor(COLOR_GRAY);
    doc.text('Zwischensumme', totalsLabelX, rowY, { width: 80, align: 'right' });
    doc.fillColor(COLOR_BLACK);
    doc.text(formatSwissAmount(data.subtotal, ''), totalsValueX, rowY, { width: totalsValueW, align: 'right' });
    rowY += 16;

    doc.fillColor(COLOR_GRAY);
    doc.text('MWST', totalsLabelX, rowY, { width: 80, align: 'right' });
    doc.fillColor(COLOR_BLACK);
    doc.text(formatSwissAmount(data.vatAmount, ''), totalsValueX, rowY, { width: totalsValueW, align: 'right' });
    rowY += 16;
    ...
    doc.text(formatSwissAmount(data.total, data.currency), totalsValueX, rowY, { width: totalsValueW, align: 'right' });
