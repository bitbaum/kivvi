...
describe("calculateTrialBalanceTotals", () => {
  it("returns all zeros for an empty trial balance", () => {
  ...
  it("sums asset balances directly (debit-normal accounts)", () => {
    // Asset account: debit > credit → positive balance
    const result = calculateTrialBalanceTotals([
      makeRow("asset", 10000, 2000), // balance = 8000
    ]);
  ...
  it("sums multiple asset accounts", () => {
    const result = calculateTrialBalanceTotals([
      makeRow("asset", 50000, 0), // cash: 50000
      makeRow("asset", 30000, 5000), // AR: 25000
    ]);
  ...
  it("takes absolute value of liability balance (credit-normal accounts)", () => {
    // Liability account: credit > debit → balance = debit - credit < 0
    const result = calculateTrialBalanceTotals([
      makeRow("liability", 500, 20000), // balance = -19500 → |19500|
    ]);
    expect(result.liabilities).toBe(19500);
  });

  it("sums multiple liability accounts", () => {
    const result = calculateTrialBalanceTotals([
      makeRow("liability", 0, 15000), // AP: 15000
      makeRow("liability", 0, 5000), // accruals: 5000
    ]);
  ...
  it("takes absolute value of equity balance (credit-normal accounts)", () => {
    const result = calculateTrialBalanceTotals([
      makeRow("equity", 0, 100000), // share capital: balance = -100000 → |100000|
    ]);
  ...
  it("computes revenue as totalCredit - totalDebit", () => {
    // Revenue is credit-normal: credit > debit
    const result = calculateTrialBalanceTotals([
      makeRow("revenue", 0, 80000), // pure revenue: 80000 - 0 = 80000
    ]);
    expect(result.revenue).toBe(80000);
  });

  it("nets revenue correctly when account has both debit and credit", () => {
    // E.g. sales returns debited back against revenue
    const result = calculateTrialBalanceTotals([
      makeRow("revenue", 5000, 85000), // net revenue = 85000 - 5000 = 80000
    ]);
    expect(result.revenue).toBe(80000);
  });

  it("sums multiple revenue accounts", () => {
    const result = calculateTrialBalanceTotals([
      makeRow("revenue", 0, 60000), // product sales
      makeRow("revenue", 0, 20000), // service revenue
    ]);
    expect(result.revenue).toBe(80000);
  });
  ...
  it("computes expenses as totalDebit - totalCredit", () => {
    const result = calculateTrialBalanceTotals([
      makeRow("expense", 30000, 0), // rent: 30000 - 0 = 30000
    ]);
    expect(result.expenses).toBe(30000);
  });

  it("nets expenses when account has credits (e.g. expense refund)", () => {
    const result = calculateTrialBalanceTotals([
      makeRow("expense", 12000, 2000), // net expense = 12000 - 2000 = 10000
    ]);
  ...
  it("correctly separates all five account types in one call", () => {
    const result = calculateTrialBalanceTotals([
      makeRow("asset", 100000, 10000), // assets: 90000
      makeRow("liability", 2000, 25000), // liabilities: 23000
      makeRow("equity", 0, 50000), // equity: 50000
      makeRow("revenue", 0, 80000), // revenue: 80000
      makeRow("expense", 30000, 0), // expenses: 30000
    ]);

    expect(result.assets).toBe(90000);
    expect(result.liabilities).toBe(23000);
  ...
  it("satisfies the balance sheet equation: assets = liabilities + equity + (revenue − expenses)", () => {
    // Balanced mini-ledger:
    //   Debit cash 10000 / Credit revenue 10000
    //   Debit expenses 3000 / Credit cash 3000
    //   Debit cash 50000 / Credit equity 50000
    //
    // Resulting balances:
    //   Cash (asset):     debit 60000, credit 3000  → balance 57000
    //   Revenue:          debit 0,     credit 10000 → net 10000
    //   Expenses:         debit 3000,  credit 0     → net 3000
    //   Equity:           debit 0,     credit 50000 → abs(−50000) = 50000
    //
    // Equation: 57000 = 0 + 50000 + (10000 − 3000) = 57000 ✓
    const result = calculateTrialBalanceTotals([
      makeRow("asset", 60000, 3000),
      makeRow("revenue", 0, 10000),
      makeRow("expense", 3000, 0),
      makeRow("equity", 0, 50000),
    ]);
  ...
  it("ignores unknown account types (only known types are aggregated)", () => {
    // Unknown type should be silently skipped
    const result = calculateTrialBalanceTotals([
      makeRow("asset", 10000, 0),
      makeRow("unknown_type", 99999, 0),
    ]);
