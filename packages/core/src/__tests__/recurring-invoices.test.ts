...
describe("variable substitution logic", () => {
  ...
  function substituteVariables(text: string, generationDate: Date, periodicity: string): string {
    const start = new Date(generationDate);
    const end = calculateNextDate(start, periodicity);
  ...
  it("handles all month names correctly", () => {
    for (let month = 0; month < 12; month++) {
      const result = substituteVariables("<%current_month%>", new Date(2026, month, 1), "monthly");
