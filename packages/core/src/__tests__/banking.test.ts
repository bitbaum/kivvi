...
import { createBankAccountSchema, importTransactionSchema } from "../domain/banking";
...
describe("QR reference matching logic", () => {
  function isQrReferenceMatch(txnReference: string | null, invoiceQrRef: string | null): boolean {
  ...
  it("matches when QR ref is embedded in longer reference", () => {
    expect(isQrReferenceMatch("Payment REF: RF123456789 from Customer AG", "RF123456789")).toBe(
      true,
    );
  });
