...
// ============================================================================
// Swiss rounding semantics — VAT rounded per line item (Swiss standard),
// Rappen rounding (nearest 0.05) applied to the grand total. All via decimal.js
// with exact expected values (Ground Truth #2: money is not a float).
// ============================================================================

describe("calculateTotals — Swiss VAT (per-line) + Rappen rounding (exact)", () => {
  it("rounds VAT at the line-item level, not on the aggregate", () => {
    // Two identical lines. Per line: 9.05 × 8.1% = 0.73305 → rounded to 0.73.
    // Sum of per-line VAT = 1.46. Rounding the aggregate instead
    // (18.10 × 8.1% = 1.4661 → 1.47) would give a different number — proving
    // the rounding happens per line, before summation (Swiss standard).
    const line = makeItem({ quantity: "1", unitPrice: "9.05", vatRate: "8.1" });
    const result = calculateTotals([line, { ...line, position: 1 }]);

    expect(result.itemTotals).toEqual(["9.05", "9.05"]);
    expect(result.subtotal).toBe("18.10");
    expect(result.vatAmount).toBe("1.46"); // 0.73 + 0.73, NOT 1.47
    // 18.10 + 1.46 = 19.56 → Rappen round (nearest 0.05) → 19.55
    expect(result.total).toBe("19.55");
  });

  it("applies Rappen rounding (nearest 0.05) to the grand total", () => {
    // 9.40 net + 0.76 VAT (9.40 × 8.1% = 0.7614 → 0.76) = 10.16.
    // 10.16 is nearer 10.15 than 10.20 → rounds down to 10.15.
    const result = calculateTotals([
      makeItem({ quantity: "1", unitPrice: "9.40", vatRate: "8.1" }),
    ]);
    expect(result.subtotal).toBe("9.40");
    expect(result.vatAmount).toBe("0.76");
    expect(result.total).toBe("10.15");
  });
