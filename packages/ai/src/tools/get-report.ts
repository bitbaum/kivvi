...
const getReportSchema = z.object({
  reportType: z.enum(['profit_and_loss', 'balance_sheet', 'vat_report', 'aging_report']).describe(
    'Type of financial report to generate.'
  ),
  startDate: z.string().optional().describe(
    'Start date in ISO format (YYYY-MM-DD). Required for P&L and VAT reports.'
  ),
  endDate: z.string().optional().describe(
    'End date in ISO format (YYYY-MM-DD). Required for P&L and VAT reports.'
  ),
});
...
export const getReportTool: Tool = {
  ...
  execute: async (params: z.infer<typeof getReportSchema>, context: ExecutionContext): Promise<ToolResult> => {
    try {
      ...
      if ((reportType === 'profit_and_loss' || reportType === 'vat_report') && (!startDate || !endDate)) {
        return {
          success: false,
      ...
      const { getProfitAndLoss, getBalanceSheet, getVatReport, getAgingReport } = await import('@kivvi/core');
