...
const recordPaymentSchema = z.object({
  documentId: z.string().uuid().describe('The UUID of the invoice or document to record a payment for'),
  amount: z.string().describe('The payment amount (e.g. "1500.00")'),
  date: z.string().optional().describe('Payment date in ISO format (YYYY-MM-DD). Defaults to today.'),
  method: z.enum(['bank_transfer', 'cash', 'card', 'other']).optional().describe('Payment method. Defaults to bank_transfer.'),
...
export const recordPaymentTool: Tool = {
  ...
  execute: async (params: z.infer<typeof recordPaymentSchema>, context: ExecutionContext): Promise<ToolResult> => {
    try {
