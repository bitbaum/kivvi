...
const convertDocumentSchema = z.object({
  ...
  targetType: z.enum([
    ...
    'invoice', 'credit_note', 'purchase_order', 'purchase_invoice',
...
export const convertDocumentTool: Tool = {
  ...
  execute: async (params: z.infer<typeof convertDocumentSchema>, context: ExecutionContext): Promise<ToolResult> => {
    try {
      const { convertDocument } = await import('@kivvi/core');

      const db = getDb(context);

      const newDoc = await convertDocument(db, context.companyId, context.userId, params.documentId, params.targetType);
