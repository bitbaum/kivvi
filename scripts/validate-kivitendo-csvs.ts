...
const CSV_FILES: Array<{ file: string; entityType: string; keyColumn?: string }> = [
  ...
  { file: 'einkaufsrechnungen_ap_invoices.csv', entityType: 'purchase_invoice', keyColumn: 'Rechnung' },
...
function validateFile(csvConfig: { file: string; entityType: string; keyColumn?: string }): ValidationResult {
  ...
  for (let i = 0; i < dataRows.length; i++) {
        ...
        if (field.transform === 'swissNumber') {
          ...
          if (colIdx >= 0) {
            ...
            if (val && !parseSwissNumber(val)) {
              badNumbers++;
              if (badNumbers <= 3) {
                issues.push(`Bad number at row ${i + 2}: "${val}" in column "${field.sourceColumn}"`);
              }
            }
          }
        }
    ...
    if (['invoice', 'purchase_invoice', 'order', 'quote', 'delivery_note'].includes(csvConfig.entityType)) {
  ...
  if (negativeAmounts > 0) issues.push(`${negativeAmounts} rows with negative amounts (will be converted to positive)`);
  if (lineItemRows > 0) issues.push(`${lineItemRows}/${validCount} rows have structured line items`);
  if (!profile) issues.push(`No auto-detection profile matched for entity type "${csvConfig.entityType}"`);

  // Build sample from first data row
  const sample: Record<string, string> = {};
  if (dataRows.length > 0) {
    const firstData = dataRows.find((r, i) => {
...
for (const csv of CSV_FILES) {
  ...
  console.log(`  Rows: ${result.dataRows} data + ${result.subtotalRows} subtotal/header = ${result.totalRows} total`);
...
const withIssues = results.filter(r => r.issues.some(i => i.startsWith('Bad') || i === 'FILE NOT FOUND')).length;
