...
async function deduplicateProducts(db: Database, companyId: string): Promise<StepResult> {
  ...
  for (const row of dupeRows as Array<{ article_number: string; cnt: string }>) {
    ...
    try {
      ...
      const dupeProducts = await db
        ...
        .from(products)
        .where(and(eq(products.companyId, companyId), eq(products.articleNumber, articleNumber)))
      ...
      await db.delete(products).where(inArray(products.id, deleteIds));
  ...
  console.log(
    `   Deduplicated ${fixed} article numbers (removed ${found > 0 ? "duplicates" : "0"} extra rows)`,
  );
...
async function removeOrphanInvoices(db: Database, companyId: string): Promise<StepResult> {
  ...
  for (const orphan of orphans) {
    try {
      ...
      await db.delete(documentItems).where(eq(documentItems.documentId, orphan.id));
...
async function fixInvoiceLineItems(db: Database, companyId: string): Promise<StepResult> {
  ...
  let fixed = 0;
  for (const doc of brokenRows as Array<{
    id: string;
    number: string;
    total: string;
    item_count: string;
  ...
  for (const row of dupeRows as Array<{ name: string; cnt: string }>) {
    ...
    try {
      ...
      const dupeWarehouses = await db
        ...
        .from(warehouses)
        .where(and(eq(warehouses.companyId, companyId), eq(warehouses.name, warehouseName)));
      ...
      await db.delete(warehouses).where(inArray(warehouses.id, deleteIds));
...
async function fixStockLevels(db: Database, companyId: string): Promise<StepResult> {
  ...
  if (existingCount === 0 && existsSync(STOCK_CSV_PATH)) {
    ...
    if (!DRY_RUN) {
      ...
      console.log(
        `   Re-imported: inserted=${result.inserted} skipped=${result.skipped} errors=${result.errors.length}`,
      );
